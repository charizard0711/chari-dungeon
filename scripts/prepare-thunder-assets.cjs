...
const art = path.join(root, 'art/thunder-runtime-v1');
const terrain = path.join(root, 'public/assets/terrain/thunder-v1');
const bossNames = ['voltyrex-directions-v1', 'spark_beetle-directions-v1', 'galvan-directions-v1', 'amatsuchi-directions-v1', 'raiga-directions-v1'];
const bossKeys = ['m_voltyrex', 'm_spark_beetle', 'm_galvan', 'm_amatsuchi', 'm_raiga'];
...
async function main() {
  ...
  for(const job of jobs) {
    ...
    if(job.kind==='boss') {
      ...
      const target=path.join(root,'public/assets/monsters/directional',bossNames[job.floor-21]+'.png');
      await canvas(256,256,layers,target);
      await sharp(target).extract({left:0,top:0,width:128,height:128}).png().toFile(path.join(root,'public/assets/monsters',bossKeys[job.floor-21]+'.png'));
      ...
      for(const {frame,...rect} of await bounds(source,3,2)) {
        if(frame<2)continue; // Replaced by full square wall drawings below.
        const size=128,maxW=112,maxH=112;
        const scale=Math.min(maxW/rect.width,maxH/rect.height),w=Math.round(rect.width*scale),h=Math.round(rect.height*scale);
        const input=await sharp(source).extract(rect).resize(w,h,{kernel:'nearest'}).png().toBuffer();
        await canvas(size,size,[{input,left:Math.round((size-w)/2),top:118-h}],path.join(dest,parts[frame]+'.png'));
      }
    } else if(job.kind==='walls') {
      const m=await sharp(source).metadata(),cw=Math.floor(m.width/2);
      for(let frame=0;frame<2;frame++) await sharp(source)
        .extract({left:frame*cw,top:0,width:cw,height:m.height})
        .resize(64,64,{kernel:'nearest'}).png({compressionLevel:9})
        .toFile(path.join(dest,parts[frame]+'.png'));
    } else await sharp(source).resize(64,64,{kernel:'nearest'}).png().toFile(path.join(terrain,'storm-clouds.png'));
